...
router.get('/', function (req, res, next) {
    request("http://" + req.hostname + "/api", function (error, response, html) {
        if (!error && response.statusCode === 200) {
            ...
            if (statusJSON === "ok") {
                ...
                res.render('serverlist', {title: 'Blockland Server List', content: serverList, playerCount: playerCount});
            } else if (statusJSON === "error") {
                var errorMessage = JSON.parse(html).message;
                res.render('serverlist', {title: 'Blockland Server List', errorMessage: errorMessage});
            }
